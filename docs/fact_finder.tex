...
\end{figure}
    ...
    \item \textit{Sign up requires user to users’ information  such as E-mail, nickname, password, password confirm. After the user signed up, they can log-in into our web service. They can use our service without login. In this case, they can't use our service without our “subscribing function of interested politician.} \\
    
    
    \begin{figure}[htbp]
\centerline{\includegraphics[width=90mm,scale=0.5]{fig/usecase5.png}}
\caption{Profile}
\label{fig}
\end{figure}


        \item \textit{In the profile page, user can change their nickname, password if they want. The changed information are stored in our database.} \\
\end{enumerate}


\item \textit{Use case 3 Looking up function about meeting contents per agenda:} They can use our Looking up function about subscribing politicians without log-in.\\
...
\begin{enumerate}
    \item\textit{Looking up list of assembly minutes:} it shows assembly minutes list. Minutes list that is classified periodically includes monthly, weekly, and certain time period setting function. Because of this, user can search the record of the past and view minutes in certain date. The minutes are classified by kinds of minutes. The kinds are like, "The main session of state affairs, standing committees, special committees, confirmation hearings, subcommittees, parliamentary audits, public hearings, hearings and joint meetings"\\
...
\end{figure}
    ...
    \item \textit{Looking up list of agendas per specific assembly minutes:}  It consists of each minutes’ count number and agendas. In the big table, it shows each count number, specifically, each table contains what agendas are discussed in that congress meeting. If user wants to search specific agenda, he can just view that  by clicking that agenda.\\
...
\end{figure}
        ...
        \item \textit{Looking up dialogue of specific agenda:} it shows detail page that belonged to chosen agenda by the user. It looks like social messenger, talking each other like shown above. Specifically, The politician’s image is attached and his or her speech is surrounded in a box. One single page is not finish until specific agenda is over.\\
\end{enumerate}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=90mm,scale=0.5]{fig/usecase12.png}}
\caption{Looking up function about subscribing politicians}
\label{fig}
\end{figure}
...
\clearpage
 ...
 \begin{figure}[htbp]
\centerline{\includegraphics[width=90mm,scale=0.5]{fig/usecase13.png}}
...
We have been working on the project for over three months. We looked at our ideas and looked for other examples that could be used to implement our ideas and designed and implemented our own webs. Since it was the first time to implement the software in person, there were many mistakes and many times we made a lot of mistakes and often spent time in inefficiencies. 
...
We had difficulties during the projects.
Our first project subject was Shared mobility service information system, but now is Political Fact finder. We had to upset all of the documentation at the requirements phase. It takes one month to fix our final subject.
After that, we had to synchronize our development environment while developing. At the first time we started developing, honestly we didn’t take much care of the compatibility of the operating system, but, while connecting between our backend and database the problem occurred. We tried to change our operating system to Ubuntu on VirtualBox and re-setting all of the development environment.
 
 \vspace{8mm}

During the implementation phase, difference of the perspective to the service makes the implementation slower. We fixed the document but understanding of the system is a lot different between developers. So we used Slack to communicate with. We talked through slack and fix the expressions, forms, conventions of our system.
 
 \vspace{8mm}

The new skills we haven’t been used for is the biggest problem during the implementation. We had to study hard to understand unfamiliar technology such as Elasticsearch, Rest API, nuxt.js. excectra.
 For example, our backend Spring should send queries with synchronized suitable methods to Elasticsearch for our perfective search system. So we should tried to use a lot of methods to find exact methods through Elasticsearch java API documentation.
 But one of the biggest our goal is to bind our own skills with new technology for our studying purpose. And we think we achieved our goal because We learned a lot how to use these new technologies.
